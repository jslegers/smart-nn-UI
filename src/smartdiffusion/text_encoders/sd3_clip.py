    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        if len(token_weight_pairs_g) > 0 or len(token_weight_pairs_l) > 0:
            if self.clip_l is not None:
                ...
                l_pooled = torch.zeros((1, 768), device=smartdiffusion.model_management.intermediate_device())
            ...
            if self.clip_g is not None:
                ...
                g_out = None
                g_pooled = torch.zeros((1, 1280), device=smartdiffusion.model_management.intermediate_device())
        ...
        if out is None:
            out = torch.zeros((1, 77, 4096), device=smartdiffusion.model_management.intermediate_device())

        if pooled is None:
            pooled = torch.zeros((1, 768 + 1280), device=smartdiffusion.model_management.intermediate_device())
