...
class ModelPatcher:
    ...
    def lowvram_load(self, device_to=None, lowvram_model_memory=0, force_patch_weights=False, full_load=False):
        ...
        for n, m in self.model.named_modules():
            ...
            if not full_load and hasattr(m, "smartdiffusion_cast_weights"):
        ...
        if unpatch_weights:
            ...
            for k in keys:
                ...
                if bk.inplace_update:
                    smartdiffusion.utils.copy_to_param(self.model, k, bk.weight)
                else:
                    smartdiffusion.utils.set_attr_param(self.model, k, bk.weight)
            ...
            if hasattr(m, "smartdiffusion_cast_weights"):
                ...
                if m.weight is not None and m.weight.device != device_to:
                    for key in [weight_key, bias_key]:
                        ...
                        if bk is not None:
                            if bk.inplace_update:
                                smartdiffusion.utils.copy_to_param(self.model, key, bk.weight)
                            else:
                                smartdiffusion.utils.set_attr_param(self.model, key, bk.weight)
                            self.backup.pop(key)
